...
以 `pro01_signal_feed` 作为 `Go_Pro` 首个完整项目，先完成本地可运行、可部署、可观测、可验收的最小闭环，并通过 Tailscale 提供简单远程访问，再逐步扩展 AI 摘要、微信通知、多来源采集和分布式部署能力。
...
最终项目应在冷启动后通过 `/healthz` 与 `/readyz`，可以在 Tailscale 网络内访问，并能完成“新增订阅源 -> 抓取 -> 查询 Feed -> 生成摘要 -> 发送通知”的闭环。
