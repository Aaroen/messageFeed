...
- LLM 客户端已支持 OpenAI-compatible 双协议智能路由：默认优先 `/responses`，不可用时降级 `/chat/completions`，成功后在客户端实例内记忆可用协议；当 `/chat/completions` 带 tools 返回空内容且无 tool_calls 时，识别为协议不匹配并自动切回 `/responses`。HTTP 429/5xx 和瞬时网络错误具备指数退避重试，非协议类 5xx 不再误触发协议切换。
- 工具协议已按 MCP 2025-11-25 原生化：内部工具目录使用 MCP `Tool` 描述，包含 `name`、`title`、`description`、`inputSchema`、`annotations`、`execution` 和 `_meta`；工具执行使用 MCP `tools/call` 语义，结果统一为 `content[]`、`structuredContent` 和 `isError`。`readOnlyHint`、`destructiveHint`、`idempotentHint`、`openWorldHint` 只作为提示，权限、预算、确认、限流、输入校验和审计仍由后端策略强制执行。当前未新增外部 MCP transport/server endpoint，OpenAI `/responses` 与 `/chat/completions` 的 function/tool 字段仅作为模型供应商适配层。
...
- 已修复子 Agent 跳过工具直接回答的问题：当主 Agent 已下发 capability scope 且当前 turn 尚无 observation 时，runner 首轮使用 `tool_choice=required`，遇到兼容服务不支持时降级为 `auto` 并继续检查真实工具调用；模型若连续返回无工具文本，会记录 `agent.required_tool_call_retry` 并最终失败为 `agent_required_tool_skipped`，避免无证据假完成。
