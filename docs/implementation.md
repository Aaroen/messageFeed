...
4. 创建每个用户默认的 `messageFeed AI` 内部源，来源类型为 `ai_agent`。
5. 将 Agent 生成的日报、周报、热点分析、主动网络研究报告、金融分析和执行结果写入 AI 源。
6. Web 支持展示 AI 源，与普通来源共用列表、详情、已读、收藏和隐藏能力。
7. Agent 执行过程接入 request id、trace id、结构化日志、指标和审计记录。
...
6. 生成日报、周报、专题摘要和热点事件分析，并写入 `messageFeed AI` 源。
7. 在 `notifier` 中抽象企业微信机器人、企业微信自建应用、`ntfy` 和后续微信通道。
8. 通知记录必须保存通道、接收目标、触发原因、状态、失败原因、模型、token、耗时和 `dedupe_key`。
...
5. 进入阶段五 Agent 基础设施：建立能力注册、结构化计划、风险校验、确认策略、执行器和审计日志。
6. 建立 `messageFeed AI` 内部源：将日报、周报、热点分析、主动网络研究报告、金融分析和 Agent 操作报告统一写入 AI 源。
7. 推进阶段六主动采集：先实现静态网页抽取、网页变化监控和搜索结果抓取评估，再接入 AI 源报告。
8. 推进阶段七推荐、摘要与通知：补齐阅读行为事件、用户画像、推荐原因、反馈闭环、摘要生成和企业微信或 `ntfy` 推送。
9. 推进阶段八金融专项：按 `docs/financial-agent-plan.md` 完成关注标的、行情快照、确定性规则、AI 解读和通知闭环。
