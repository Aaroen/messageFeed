...
当前活动文档为 `docs/nowdoit/agent-session-service-snapshot-recorder-modularization-plan.md`。下一轮转入 `agent_session_service.go` 治理，优先迁出只写审计快照、不改变任务聚合业务语义的 `recordAgent*Snapshot` 方法群组，并新增独立 `internal/service/agent_session_snapshot_recorders.go`。
