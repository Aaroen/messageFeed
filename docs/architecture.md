...
每次真正唤起 Agent 执行任务时，系统应创建隔离 `AgentRun`。长期企微 session 只作为入口、账号映射和 transcript 主事实源，不默认成为模型执行上下文。`AgentRun` 通过 `TaskPacketBuilder` 注入用户本次任务、必要用户快照、能力边界、当前时间、预算和少量相关上下文；如需长期聊天历史，必须通过 `conversation.query_history` 等能力按需召回。`AgentContextManager` 负责组装冻结记忆快照、管理 transcript 冷热归档索引和召回原文；`ContextBudgetManager` 负责模型可见上下文投影视图、token 估算、工具结果裁剪、压缩摘要和工具调用对保护。归档索引不得替代 `agent_transcript_entries` 原文，摘要只能作为临时展示或模型当前轮输出，不作为长期事实源。
...
| `A2A` | Message/Task 分层、Task 生命周期、Agent Card、异步和推送 | 将复杂 `AgentRun` 建模为可跟踪 task，后续外部协作可参考 Agent Card | 阶段五不开放外部 Agent 服务 |
| `LangGraph` | state graph、checkpoint、interrupt/resume、retry | `RunLoop` 保存 step checkpoint，用户确认和补充信息作为可恢复中断 | 不把图框架作为业务主模型 |
| `AutoGen` | 消息驱动 runtime、team/group chat、termination condition | 后续多 Agent 协作可作为内部编排，执行循环必须有终止条件 | 普通企业微信对话不默认进入多 Agent team |
| `browser-use`、`gpt-researcher` | 浏览器动作空间、allowed domains、深度研究、来源引用 | 联网研究能力分层为搜索、抓取、抽取、受控浏览器、仓库搜索和受控克隆 | 不直接引入完整浏览器 Agent 或 Python 运行时 |
...
AI Agent 采用“Web 命令、企业微信入站消息或系统事件 -> AgentTrigger -> 隔离 AgentRun/turn -> 任务包与冻结快照 -> 模型路由与上下文适配估算 -> 能力检索 -> 结构化计划 -> 策略校验 -> 受控 RunLoop -> AI 源沉淀 -> 对话回复或通知反馈”的链路：
...
3. `AgentRunManager` 创建隔离 `AgentRun` 和 turn；企微长期 session 只作为来源引用和结果回写目标。
...
8. `agent` 调用 `llm` 解析意图，优先匹配核心能力；能力不足时通过 `capability.search` 检索延迟能力。
9. `agent` 生成 `agent_plans` 和 `agent_plan_steps`，标注风险等级、确认策略、影响范围、幂等键和模型策略。
10. `PolicyEngine` 将计划决策为 `allow`、`prompt` 或 `forbidden`；`prompt` 操作要求用户确认。
11. 用户确认后，`RunLoop` 在受控轮次、token、工具次数和时间预算内执行 step、接收 observation、继续、重规划、追问或失败。
12. `AgentExecutor` 只能调用已注册能力和既有 service 接口，不直接写数据库；`ContextBudgetManager` 在每次模型调用前后管理上下文投影视图。
13. 联网研究只通过 `web.search`、`web.fetch_page`、`web.extract_page`、`web.browse_page`、`repo.search`、`repo.inspect_remote` 和 `repo.clone_reference` 等能力执行。搜索结果必须经抓取、抽取或远端仓库检查后才能作为证据；`repo.clone_reference` 只能浅克隆到受控 `references/` 目录。
14. Agent 生成的日报、周报、热点分析、主动网络研究报告和执行结果写入 `messageFeed AI` 内部源；企业微信普通聊天只进入 transcript 和 audit。
15. 企微历史聊天完整保存在 transcript 中；较早记录从热上下文移出后进入冷热归档索引，模型需要回忆时通过历史查询能力取回原文片段。
16. 企业微信对话回复由当前 turn 的响应链路发送；高优先级 AI 源条目或告警由阶段七通知系统通过企业微信自建应用消息、可选智能机器人主动消息、`ntfy` 等通道推送。
17. 全过程写入 transcript、审计日志、归档索引、记忆召回记录、模型选择、token 使用、指标和 trace。
