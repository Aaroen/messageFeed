...
所有 Agent 可执行能力必须注册到 `AgentCapabilityRegistry`。模型只能生成意图、计划、说明文本和工具参数摘要；实际执行必须由 `AgentExecutor` 调用已注册能力和既有 service 接口完成。能力暴露分为 `core`、`deferred`、`hidden`：少量核心能力默认进入模型上下文，延迟能力通过 `capability.search` 发现后再暴露 schema，隐藏能力只能由后端策略或已确认计划内部调用。

每次真正唤起 Agent 执行任务时，系统应创建 controller `AgentRun`。长期企微 session 只作为入口、账号映射和 transcript 主事实源，不默认成为模型执行上下文。`ControllerAgent` 是唯一主控，负责理解用户意图、拆分任务、选择能力、创建 executor `AgentRun`、汇总结果、判断继续或结束，并生成最终回复。executor `AgentRun` 可无限创建、即用即丢、上下文隔离，只负责一个明确任务包；执行完成后销毁运行上下文，但必须持久保存完整模型可见上下文、工具上下文、observation、artifact 和审计记录。`AgentContextManager` 负责组装冻结记忆快照、管理 transcript 冷热归档索引和召回原文；`ContextBudgetManager` 负责模型可见上下文投影视图、token 估算、工具结果裁剪、压缩摘要和工具调用对保护；`ContextTraceStore` 负责保存 controller 与 executor 的上下文追溯记录。归档索引不得替代 `agent_transcript_entries` 原文，摘要只能作为临时展示或模型当前轮输出，不作为长期事实源。

模型选择通过 `ModelRouter` 完成。强模型用于复杂任务规划、能力选择、任务拆解、失败重规划和长上下文判断；普通模型用于简单任务执行、固定提醒、摘要和格式化；轻量模型用于分类、标签、索引和低风险判断。模型元数据应持久化或配置化，至少记录 provider、模型名、角色、上下文窗口、最大输出、工具调用能力、结构化输出能力、成本、延迟等级、可靠性和启用状态。完整 Agent 方案见 `docs/agent-plan.md`。
...
| `opencode` | durable inbox、Context Epoch、ToolRegistry、输出裁剪与 settlement | 输入先持久化再提升为模型可见消息；工具注册、执行和结果边界分离 | 不引入其 Effect/TypeScript 运行时 |
| `A2A` | Message/Task 分层、Task 生命周期、Agent Card、异步和推送 | 将 controller/executor `AgentRun` 建模为可跟踪 task，后续外部协作可参考 Agent Card | 阶段五不开放外部 Agent 服务 |
| `LangGraph` | state graph、checkpoint、interrupt/resume、retry | `RunLoop` 保存 step checkpoint，用户确认和补充信息作为可恢复中断 | 不把图框架作为业务主模型 |
| `AutoGen` | 消息驱动 runtime、team/group chat、termination condition | 多 Agent 协作收敛为唯一 ControllerAgent 调度临时 ExecutorAgentRun，执行循环必须有终止条件 | 不引入常驻多角色 team |
| `browser-use`、`gpt-researcher` | 浏览器动作空间、allowed domains、深度研究、来源引用 | 联网信息获取能力分层为搜索、抓取、抽取、受控浏览器、仓库搜索和受控克隆 | 不直接引入完整浏览器 Agent 或 Python 运行时 |
...
AI Agent 采用“Web 命令、企业微信入站消息或系统事件 -> AgentTrigger -> controller AgentRun/turn -> 任务包与冻结快照 -> 模型路由与上下文适配估算 -> ControllerAgent 规划 -> executor AgentRun 执行 -> observation/artifact 汇总 -> 策略校验 -> AI 源沉淀 -> 对话回复或通知反馈”的链路：
...
3. `AgentRunManager` 创建 controller `AgentRun` 和 turn；企微长期 session 只作为来源引用和结果回写目标。
4. `TaskPacketBuilder` 从入口消息、系统事件或定时契约构造任务包。
5. `ModelRouter` 读取模型元数据，选择轻量分类模型、强规划模型、普通执行模型或总结模型。
6. `ContextFitEstimator` 预估工具次数、工具结果大小、模型轮次、输入输出 token 和单上下文可完成性。
7. `AgentContextManager` 组装冻结用户快照、任务约束、相关条目、AI 源报告、网页快照和能力边界；长期聊天历史只按需召回。
8. `ControllerAgent` 调用 `llm` 解析意图，优先匹配核心能力；能力不足时通过 `capability.search` 检索延迟能力。
9. `ControllerAgent` 生成 `agent_plans`、`agent_plan_steps` 和若干 executor task，标注风险等级、确认策略、影响范围、幂等键、模型策略和 capability scope。
10. `PolicyEngine` 将计划决策为 `allow`、`prompt` 或 `forbidden`；`prompt` 操作要求用户确认。
11. 用户确认后，`RunLoop` 按计划创建一个或多个 executor `AgentRun`。每个 executor 只接收明确任务包、允许能力范围、预算和必要上下文。
12. `AgentExecutor` 只能调用已注册能力和既有 service 接口，不直接写数据库；`ContextBudgetManager` 在每次模型调用前后管理上下文投影视图。
13. `ContextTraceStore` 必须保存每个 controller/executor run 的模型可见上下文、工具 schema 投影视图、工具调用、observation、artifact 引用、token 估算和裁剪记录；敏感配置不得进入上下文快照。
14. 联网信息获取只通过 `web.search`、`web.fetch_page`、`web.extract_page`、`web.browse_page`、`repo.search`、`repo.inspect_remote` 和 `repo.clone_reference` 等能力执行。搜索结果必须经抓取、抽取或远端仓库检查后才能作为证据；`repo.clone_reference` 只能浅克隆到受控 `references/` 目录。
15. Agent 生成的日报、周报、热点分析、主动网络信息获取报告和执行结果写入 `messageFeed AI` 内部源；企业微信普通聊天只进入 transcript 和 audit。
16. 企微历史聊天完整保存在 transcript 中；较早记录从热上下文移出后进入冷热归档索引，模型需要回忆时通过历史查询能力取回原文片段。
17. 企业微信对话回复由当前 turn 的响应链路发送；高优先级 AI 源条目或告警由阶段七通知系统通过企业微信自建应用消息、可选智能机器人主动消息、`ntfy` 等通道推送。
18. 全过程写入 transcript、审计日志、归档索引、上下文追溯记录、记忆召回记录、模型选择、token 使用、指标和 trace。
