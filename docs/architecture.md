...
| 企业微信与通知 | 企业微信智能机器人短连接回调作为对话入口；企业微信自建应用、智能机器人主动消息、ntfy 和后续公众号作为通知出口 |
| 日志 | log/slog |
...
| `channel` | 管理企业微信智能机器人、Web、后续移动端等入站消息协议适配、验签解密和消息标准化 |
...
| `notifier` | 抽象 ntfy、企业微信自建应用、智能机器人主动消息和后续通知出口 |
...
企业微信智能机器人入站消息不进入 `notifier`。`handler` 负责 HTTP 路由入口，`channel/wechatwork` 负责 URL 验证、签名校验、AES 解密、消息标准化和通道回复，`agent` 负责 session/turn、只读 Runner、审计和后续能力执行。企业微信自建应用或智能机器人主动推送属于通知出口，进入 `notifier` 和阶段七通知系统。
...
| `external_accounts` | 外部账号映射，包含企业微信智能机器人或自建应用的外部用户 ID、provider、系统用户 ID 和绑定状态 |
| `agent_inbound_messages` | Agent 入站消息，记录企业微信 `msgid`、外部用户、会话类型、原始 payload、处理状态、request id 和 trace id |
...
| `GET` | `/api/v1/channels/wechat-work/aibot/callback` | 企业微信智能机器人 URL 验证 |
| `POST` | `/api/v1/channels/wechat-work/aibot/callback` | 企业微信智能机器人消息回调 |
...
AI Agent 采用“Web 命令、企业微信入站消息或系统事件 -> session/turn -> 上下文与记忆快照 -> 意图解析 -> 能力检索 -> 结构化计划 -> 策略校验 -> 用户确认 -> 受控执行 -> AI 源沉淀 -> 对话回复或通知反馈”的链路：

1. `handler` 接收 Web 自然语言命令、系统事件或企业微信回调。企业微信回调先进入 `channel/wechatwork` 完成 URL 验证、签名校验、AES 解密、消息标准化和 `msgid` 幂等。
...
4. `AgentContextManager` 组装冻结记忆快照，读取用户画像、当前任务、相关条目、AI 源报告、网页快照和能力边界。
...
9. Agent 生成的日报、周报、热点分析、主动网络研究报告、对话处理报告和执行结果写入 `messageFeed AI` 内部源。
10. 当上下文达到压缩阈值时，按完整语义块归档历史，保留摘要、索引、窗口编号、压缩前后 token 和原文引用。
11. 企业微信对话回复由当前 turn 的响应链路发送；高优先级 AI 源条目或告警由阶段七通知系统通过企业微信自建应用、智能机器人主动消息、`ntfy` 等通道推送。
