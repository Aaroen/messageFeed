...
企业微信 Agent 对话采用完整 transcript 原文保存、短期聊天窗口自动注入和历史聊天原文按需查询的设计。普通企微聊天记录不进入 `messageFeed AI` 源、订阅主页或推荐主页；归档只作为冷热检索索引，不替代原文。
...
- [Agent 方案](docs/agent-plan.md)：定义项目级 Agent、企业微信对话入口、上下文记忆、能力治理和评测方案。

## 当前结论

当前已收敛为本地单节点运行、Cloudflare Tunnel 对外入口和生产静态 Web 服务。`DEPLOYMENT_MODE` 表示部署拓扑，不表示访问范围；当前公开入口为 `PUBLIC_BASE_URL` 对应的 Cloudflare 域名，本机统一入口为 `https://localhost:8443`，局域网和 Tailscale 直连入口关闭。
